...
def _member_dashboard_context(member_id=None):
    ...
    indicators = [
        getattr(session, "indicator", None) for session in chronological_sessions
    ]
...
def dashboard_context(member_id=None):
    ...
    counts = {
        key: sum(1 for s in sessions if s.route == key)
        for key in ["AUTO", "REVIEW", "BLOCK"]
    }
    qc_counts = {
        key: sum(1 for s in sessions if s.qc_status == key)
        for key in ["PASS", "CHECK", "FAIL"]
    }
