...
class DashboardPrivacyAndContextTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        for index, status in enumerate(statuses):
            session = Session.objects.create(
                ...
                session_id=f"synthetic-session-{index}",
                session_date=(
                    timezone.now() - timezone.timedelta(days=2 - index)
                ).date(),
    ...
    def test_selected_member_context_contains_dashboard_data(self):
        response = self.client.get(
            reverse("dashboard"), {"member_id": self.member.member_id}
        )
        self.assertEqual(
            response.context["selected_member_id"], str(self.member.member_id)
        )
    ...
    def test_dashboard_renders_safety_copy_without_pii(self):
        ...
        self.assertContains(
            response,
            "진단·치료 목적이 아니라 PT 상담과 웰니스 피드백을 위한 프로토타입",
        )
    ...
    def test_font_family_fallback_stack_exists_in_css(self):
        ...
        css_text = (
            ...
            .resolve()
            .parents[1]
            .joinpath("static/lightone/css/lightone.css")
            .read_text(encoding="utf-8")
        )
        self.assertIn(
            'font-family: Inter, "Noto Sans KR", "Pretendard", system-ui, -apple-system, BlinkMacSystemFont, sans-serif;',
            css_text,
        )
